...
for (f,g1,g2) in math2arg
    # bf = broadcast_func(f)
    @eval @primitive2 $f(x1,x2),dy,y  unbroadcast(x1,dy.*($g1))  unbroadcast(x2,dy.*($g2))
    # if f != bf
