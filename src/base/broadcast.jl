...
(:./, :(dy./x2), :(-dy.*x1./abs2_dot(x2))),
(:.\, :(-dy.*x2./abs2_dot(x1)), :(dy./x1)),
(:.^, :(dxndx(x1,x2,dy)), :(dy.*y.*log_dot(x1))), # domain: x1 >= 0 (unless we use complex args)
...
function addtest_broadcast(f,r1,r2=r1)
    ...
    addtestN(f,randin(r1),randin(r2,2))
    addtestN(f,randin(r1,2),randin(r2))
    addtestN(f,randin(r1,2),randin(r2,2))
    addtestN(f,randin(r1,2),randin(r2,2,2))
    addtestN(f,randin(r1,2,2),randin(r2,2))
    addtestN(f,randin(r1,1,2),randin(r2,2,2))
    addtestN(f,randin(r1,2,2),randin(r2,1,2))
end
