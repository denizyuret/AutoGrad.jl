...
# 2. g calls forward_pass which boxes x in a Box type and calls f(Box(x)).
# 3. If a primitive operator inside f gets a Box input, it records its action and returns a Box output.
...
# 2. g calls forward_pass which boxes argnum'th arg x in a Box type and calls f(Box(x))
# 2.1 f must be defined generically to accept Box arguments.
...
# 2.6 the output of f (end_box) could be a boxed Box or a regular value (if it does not depend on x)
...
# forward_pass deps: Tape, Box, Node


# 3. If a primitive operator inside f gets a Box input, it records its action and returns a Box output.

# 3.1 We implement this by dispatching f to r=recorder(f) if any of
# its arguments is a Box.  r unboxes the arguments, calls f, boxes
...
have at least one Box argument.
"""
function recorder(f)
r = get(fdict,f,0)
r != 0 && return r
...
function rfun(args...; kwargs...)
    ...
    for argnum = 1:length(args)
        arg = args[argnum]
        isa(arg,Box) || continue
        for t=1:length(arg.tapes)
            tape = arg.tapes[t]
            iscomplete(tape) && continue
            parent = arg.nodes[t]
            if !isa(result,Box) 
                result = Box(result, tape; func=f, args=args, kwargs=kwargs)
...
# recorder deps: Box, Node, iscomplete, findeq

"getval(x) unboxes x if it is a Box, otherwise returns x."
getval(x) = (if isa(x, Box); x.value; else; x; end)  # we never create Box(Box).
...
# 4.1 backward_pass is called with start_box: Box(x), end_box:
# f(Box(x)) (which may or may not be a boxed Box), and the tape
# created by the corresponding forward_pass.  Note that Box(x) may
...
# 4.2 If end_box is not a Box on the given tape, we return zero
# df/fx if x is a bits type, `nothing` otherwise.  end_box may not
# be a Box if the output of f does not depend on x.

    if !isa(end_box, Box) || 0==(tapeidx=findeq(end_box.tapes, tape))
...
# output Box whose gradient is set to 1.0.
...
# Boxs and recorded on the tape. In the beginning the only non-empty
# outgrad is the one for the end_box.  Since the end_box is a
# boxed Box (otherwise we returned 0/nothing), it must depend on
# input x.  The input is the first thing recorded on tape by
# forward_pass, thus will be the last thing whose gradient is seen.
# If there are Boxs influenced by x but do not influence the
...
# 5.1 Box: g=grad(f) calls forward_pass which calls f with one
# argument boxed in a Box type.  The primitives inside f call their
# recorder methods when one of their arguments is a Box.  The results
# of these recorder methods are also boxed in Box types, which will
# cause downstream primitives to be recorded as well.  The final
# output of f, if not independent of the input, will thus be a Box.

# 5.2 Node: Each result Box created by a primitive keeps track of
# the function and the arguments that created the Box.  Because a
# Box may need to be recorded in multiple tapes for higher order
...
function Box(value, tape::Tape=Tape(); func=rand, args=(), kwargs=[])
    self = Box(value,func,args,kwargs,Tape[tape],Array(Node,1))
    node = Node(self)
    push!(tape,node)
    self.nodes[1] = node
    return self
...
function Node(b::Box, t::Tape) # assumes b is not already in t
    n = Node(b)
    push!(t, n)
    push!(b.nodes, n)
    push!(b.tapes, t)
...
# Primitives with Box arguments may be called during the
...
# some arguments boxed in Boxs (However, see 6.3 for
...
# method where some arguments are boxed in Boxs are directed to the
...
# is no easy way to say "at least one argument is a Box" in Julia.
...
# `f{T<:Number,A<:AbstractArray{T}}(Box{A})` yet.
...
# their action or return a boxed Box.  They can just unbox their
# arguments and return an unboxed value:

# `size(a::Box,i...)=size(a.value,i...)`
...
#     sum{T<:Array}(a::Box{T},i::Int)=sum_r(a,i)
#     sum{T<:Array}(::Type{Grad{1}},dy,y,a::Box{T},i::Int)=dy.+zeros(a)
...
sum_outgrads(a::Box,b::Box)=sum_outgrads_r(a,b)
sum_outgrads(a::Box,b)=sum_outgrads_r(a,b)
sum_outgrads(a,b::Box)=sum_outgrads_r(a,b)
sum_outgrads{N}(::Type{Grad{N}},dy,y,x1,x2)=dy
# we use `nothing` to indicate zero gradients
sum_outgrads(::Void,::Void)=nothing
sum_outgrads(a::Box,::Void)=a   # to avoid ambiguity
sum_outgrads(::Void,a::Box)=a   # to avoid ambiguity
...
# merge_tapes in forward_pass(g,x) is a noop because x is not a Box.
# forward_pass(g,x) wraps x in v1=Box(x,t1:n1) with tape t1 and node n1 and calls g(v1).
# g(v1) calls forward_pass(f,v1), which creates v2=Box(x,t2:n2)
# merge_tapes in forward_pass(f,v1) creates v3=Box(x,[t1:n31,t2:n32]) with parents n31->n1, n32->n2.
...
# f(v3) returns v4=Box(y,[t1:n41,t2:n42]).
...
# backward_pass(f) returns v5=Box(df/dx,t1:n5) which becomes the output of forward_pass(g,x)
# h(x) calls backward_pass(g)(v1,v5,t1).
# backward_pass(g) calls gradient methods recorded in t1.
# even though some inputs are Boxs again, nothing gets recorded and all primitives return values because t1 is complete.
