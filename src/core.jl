...
`g(X...)->dXi` which computes the gradient of `f` with respect to
positional argument number `argnum`. The function `f` should be
scalar-valued. The returned function `g` takes the same arguments as
...
"""
    ...
    forward_pass(f, args, kwargs, argnum)

Mark the target argument `args[argnum]` 
Call function `f` with arguments `args` and keyword arguments `kwargs`.  
...
(see Sec. 7) these dependencies are kept in a separate data structure
called a Node.  The parents field of a Node is an array that points to
the Nodes of the arguments.  The gradient wrt the result is kept in
the outgrad field.  outgrad is an array because a node can have
multiple descendents each of which will push a gradient to outgrad to
be summed.
...
backward_pass. We do not want those primitives being recorded any more
(at least on the tape created by the corresponding forward_pass, see
Sec 7 for details).  We stop the recording on a Tape by calling its
complete! method.
