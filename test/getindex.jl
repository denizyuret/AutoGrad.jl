...
@testset "getindex" begin
    for ft in [Float32,Float64]
        @testset "$(ft)" begin
            ...
            @testset "Dict" begin
                g = grad(getindex)
                d = Dict(1=>rand(ft), 2=>rand(ft), 3=>rand(ft))
                #@show d
                @test_skip gradcheck(getindex, d, 1) # TODO: gradcheck with dict broken
                @test collect(g(d,2)) == Any[(2=>ft(1.0))]
            end

            @testset "size" begin
                #@show size
                f0(x)=(p=size(x); p[1]*sum(abs2,x))
                @test grad(f0)(ones(ft,3)) == fill(6, 3)

                f1(x)=(p=size(x, 1); p*sum(abs2,x))
                @test grad(f1)(ones(ft,3, 3)) == fill(6, 3, 3)
    ...
    =#
            ...
            @testset "indexing" begin
                ...
                a1 = rand(ft,3)                    # TODO: FAIL: some high-order (b1sum) tests with rand(2)
                t1 = (a1...,)
